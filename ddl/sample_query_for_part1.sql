...
select count(a.uid)
from 
tasks p inner join tasks i on p.tid = i.parent_tid
...
tasks i inner join wf_state ws on i.wfid = ws.wfid and i.status = ws.from_status
inner join status s on ws.to_status = s.sid
where i.tid = 2/* issue id*/
;
...
where tid = 2 /* issue id */
order by created_ts desc
;

/* List any issues for the project with name “Amazon Kindle” where the issue title contains the
term “screen”, user “Jeff Bezos” is one of the assignees, and the status of the issue is
“OPEN”.  */

select * from
tasks i inner join tasks p on i.parent_tid = p.tid
inner join assignment a on a.tid = i.tid
inner join users u on a.uid = u.uid
inner join status s on i.status = s.sid
where
p.title = 'Amazon Kindle' 
and MATCH(i.title) against ('screen' IN NATURAL LANGUAGE MODE)
and u.uname = 'Jeff Bezos'
;