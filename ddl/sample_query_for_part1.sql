...
insert into tasks (ttype, parent_tid, reporter, title, description, wfid, status, created_ts ) values 
('P', 1, 5, 'Stay At Home', 'I hereby declare a state of emergency in GNV', 1, 1, now());

select * from tasks where ttype = 'P';

set @new_tid = (select LAST_INSERT_ID());   /*what is this*/ 
...
from tasks i inner join wf_state ws on i.wfid = ws.wfid and i.status = ws.from_status
...
and MATCH(i.title) against ('screen' IN NATURAL LANGUAGE MODE) and u.uname = 'Jeff Bezos'  /*what is this*/ 
;